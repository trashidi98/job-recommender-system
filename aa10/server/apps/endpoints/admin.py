...
class BestJobsOutputAdmin(ImportExportModelAdmin):
    ...
    list_display = ("job_id", "job_title", "job_category", "company_name", "inferred_city",
                    "inferred_state", "inferred_country", "job_description", "job_type", "similarity_score")
...
class MLAlgorithmStatusAdmin(ImportExportModelAdmin):
    ...
    list_display = ("id", "active", "status", "created_by",
...
class MLRequestAdmin(ImportExportModelAdmin):
    model = MLRequest
    list_display = ("id", "input_data", "full_response", "response",
